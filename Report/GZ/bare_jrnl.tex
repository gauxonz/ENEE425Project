...
\hline 852 & 7 & 8 & 9 & C \\
\hline 941 & $\star$ & 0 & $\#$& 0 \\
\hline \end{tabular} \caption{DTMF Keypad Frequencies Match }\end{center}\label{table_kpd}\end{table}
\subsection{FFT}
Fast Fourier Transform( FFT) is an algorithm to make computation of Discrete Fourier Transform(DFT) faster. By DFT, signals are converted from time domain to frequency domain, which makes it easier to do analysis. In this project, due to effect of noise, after DFT, there may be more than two frequencies having non-zero amplitude, but two of them should be relatively larger and they are actually the target frequencies.\\
\indent The following figure shows an instance of the FFT of signal `5', two of the frequency peak can be easily found:
\begin{figure}[!t]
\centering
\includegraphics[width=2.5in]{dig5.jpg}
 %\DeclareGraphicsExtensions.
\caption{Example FFT of dialing `5'}
\label{fig_dail5}
\end{figure}
\section{Proposed Approach}
The decoder is accomplished following the flow chart bellow:
...
\begin{tikzpicture}[node distance = 2cm, auto,scale=0.6, every node/.style={transform shape}]
    % Place nodes
    \node [cloud] (file) {.wav file};
    \node [block,below of=file,node distance=1.5cm](read){Read file};
...
\end{tikzpicture}
\caption{Flow Chart for the decoder} \label{fig:fc}
\end{figure}
